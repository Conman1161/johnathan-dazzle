...
class BotInfoCommand extends slash_create_1.SlashCommand {
    ...
    async run(ctx) {
        ...
        let embed = new discord_js_1.MessageEmbed()
            ...
            .addField(`${process.env.CATBOY || ctx.options.force_catboy ? `${owoify('Bot Info')}` : `Bot Info`}`, `${process.env.CATBOY || ctx.options.force_catboy ? `${owoify(`Hello, I'm Johnathon Dazzle! I roll the dice around here. I've got a few tricks up my sleeve, so check them out with`)}` : `Hello, I'm Johnathon Dazzle! I roll the dice around here. I've got a few tricks up my sleeve, so check them out with`} \`/help\`!`)
            .addField(`${process.env.CATBOY || ctx.options.force_catboy ? `${owoify(`__**WORK IN PROGRESS**__`)}` : `__**WORK IN PROGRESS**__`}`, `${process.env.CATBOY || ctx.options.force_catboy ? `${owoify(`Dazzle is currently in development, meaning some functions and features are not finished or completely implemented yet. Please keep this in mind when using Dazzle`)}` : `Dazzle is currently in development, meaning some functions and features are not finished or completely implemented yet. Please keep this in mind when using Dazzle`}`)
            .addField(`${process.env.CATBOY || ctx.options.force_catboy ? `${owoify(`Bot Support`)}` : `Bot Support`}`, `${process.env.CATBOY || ctx.options.force_catboy ? `${owoify(`If you find an issue, message __**${process.env.OWNER_TAG}**__ with a screenshot and a short description of the issue or join`)} [this](https://discord.gg/ZUJAMnh) ${owoify('discord server')}`
            ...
            .addField(`${process.env.CATBOY || ctx.options.force_catboy ? `${owoify(`Command Ideas`)}` : `Command Ideas`} `, `${process.env.CATBOY || ctx.options.force_catboy ? `${owoify(`If you have any ideas for commands, message __**${process.env.OWNER_TAG}**__ with them`)}` : `If you have any ideas for commands, message __**${process.env.OWNER_TAG}**__ with them`} `, true)
            .addField(`${process.env.CATBOY || ctx.options.force_catboy ? `${owoify(`Bot Version`)}` : `Bot Version`} `, `${process.env.CATBOY || ctx.options.force_catboy ? `${owoify(`Currently running v**${package_json_1.version}**`)}` : `Currently running v**${package_json_1.version}**`} `)
        ...
        await ctx.send({
            ...
            file: {
                ...
                file: fs_1.readFileSync(process.env.CATBOY || ctx.options.force_catboy ? `./images/catboy/Background.png` : `./images/icon.png`)
            },
            components: [{
